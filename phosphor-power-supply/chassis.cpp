...
void Chassis::getPSUConfiguration()
{
    ...
    for (const auto& [path, services] : objects)
    {
        ...
        std::string currentBackplane = nisqually + chassisId;

        if (path.empty() || service.empty() ||
            (!matchesThisChassis(path, currentBackplane)))
        {
            continue;
        }
