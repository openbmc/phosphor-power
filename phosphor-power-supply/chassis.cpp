...
void Chassis::getPSUConfiguration()
{
    ...
    getProperty(decoratorChassisId, "SlotNumber", chassisPath,
    ...
    try
    {
        auto decoratorSubTree = getSubTree(bus, "/", decoratorChassisId, depth);
        for (const auto& [objectPath, services] : decoratorSubTree)
        {
            for (const auto& [service, interfaces] : services)
            {
                if (service == entityMgrService)
                {
                    auto connectorsSubTree =
                        getSubTree(bus, "/", IBMCFFPSInterface, depth);
                    for (const auto& [path, services] : connectorsSubTree)
                    {
                        uint64_t id;

                        getProperty(decoratorChassisId, "SlotNumber",
                                    objectPath, service, bus, id);
...

                        {
                            ...
                            auto properties = getAllProperties(
                                bus, path, IBMCFFPSInterface, service);
