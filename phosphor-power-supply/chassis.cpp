...
Chassis::Chassis(sdbusplus::bus_t& bus, const std::string& chassisPath) :
    bus(bus), chassisPath(chassisPath), chassisId(chassisPath.back())
{
    getPSUConfiguration();
}
...
void Chassis::getPSUProperties(util::DbusPropertyMap& properties)
{
    ...
    // path.
    std::string baseChassisInvPath =
        "/xyz/openbmc_project/inventory/system/chassis";
    baseChassisInvPath += chassisId;
    std::string basePSUInvPath =
        baseChassisInvPath + "/motherboard/powersupply";
...
void Chassis::populateChassisProperties(const util::DbusPropertyMap& properties)
{
    try
    {
