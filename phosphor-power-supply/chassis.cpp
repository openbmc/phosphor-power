...
const auto entityMgrIface = "xyz.openbmc_project.EntityManager";
const auto decoratorSlot = "xyz.openbmc_project.Inventory.Decorator.Slot";
...
void Chassis::getPSUConfiguration()
{
    ...
    auto chassisObject =
        getSubTree(bus, "/xyz/openbmc_project/inventory/system/board",
                   decoratorSlot, depth);

    chassisSlotNumber = getSlotNumber(INVENTORY_MGR_IFACE, chassisPath);

    for (const auto& [path, services] : chassisObject)
    {
        bool found = false;
        try
        {
            auto boardSlotNumber = getSlotNumber(entityMgrIface, path);
            ...
            for (const auto& [path, services] : ibmCFFobjects)
            {
                ...
                auto backplanePath = path.substr(0, path.rfind("/"));

                auto backplaneSlotNumber =
                    getSlotNumber(entityMgrIface, backplanePath);

                if ((chassisSlotNumber == backplaneSlotNumber) &&
                    (chassisSlotNumber == boardSlotNumber))
                {
...
uint64_t Chassis::getSlotNumber(const std::string& service,
                                const std::string& objectPath)
{
    ...
    try
    {
        ...
        method.append(decoratorSlot, "SlotNumber");
