...
void Chassis::getPSUConfiguration()
{
    using namespace phosphor::power::util;
    auto depth = 0;
    auto ibmCFFobjects = getSubTree(bus, "/", IBMCFFPSInterface, depth);

    chassisPathUniqueId =
        getObjectPathUniqueId(INVENTORY_MGR_IFACE, chassisPath);
    ...
    for (const auto& [path, services] : ibmCFFobjects)
    {
        try
        {
            auto service = services.begin()->first;
            auto backplanePath = extractBackplanePath(path);
            if (path.empty() || service.empty() || backplanePath.empty())
            {
                continue;
            }

            auto backplanePathUniqueId =
                getObjectPathUniqueId(entityMgrService, backplanePath);

            if (chassisPathUniqueId == backplanePathUniqueId)
            {
                // For each object in the array of objects, I want to get
                // properties from the service, path, and interface.
                auto properties =
        ...
        {
            lg2::error(
                "Failed while getting configuration - exception: {ERROR}",
                "ERROR", e);
        }
    }
...
uint64_t Chassis::getObjectPathUniqueId(const std::string& service,
                                        const std::string& objectPath)
{
    std::variant<uint64_t, uint32_t> propValue;
    uint64_t objectPathId(invalidObjectPathUniqueId);
    try
    {
        auto method = bus.new_method_call(service.c_str(), objectPath.c_str(),
                                          PROPERTY_INTF, "Get");

        method.append(decoratorChassisId, "SlotNumber");
    ...
    {
        lg2::error("failed to get {OBJECT_PATH} slot number {ERROR} ",
                   "OBJECT_PATH", objectPath, "ERROR", e);
        return objectPathId;
    }

    const uint64_t* val64_ptr = std::get_if<uint64_t>(&propValue);
    if (val64_ptr != nullptr)
    {
        objectPathId = *val64_ptr;
    }
    else
    {
        const uint32_t* val32_ptr = std::get_if<uint32_t>(&propValue);
        if (val32_ptr != nullptr)
        {
            objectPathId = *val32_ptr;
        }
    }
    return objectPathId;
}

std::string Chassis::extractBackplanePath(const std::string& path)
{
    const std::string keyword = "Backplane";
    size_t pos = path.find(keyword);
    if (pos == std::string::npos)
        return {};

    // Find the start of the segment containing "Backplane"
    size_t start = path.rfind('/', pos);
    if (start == std::string::npos)
    {
        return {}; // Shouldn't happen for valid paths
    }
