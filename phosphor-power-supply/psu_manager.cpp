                             ...
                             std::map<std::string, std::string> additionalData)
{
    ...
    {
        log<level::ERR>((std::string("Failed creating event log for fault ") +
                         faultName + std::string(" due to error: ") + e.what())
                            .c_str(),
                        entry("PATH=%s", loggingObjectPath),
                        entry("INTERFACE=%s", loggingCreateInterface));
    }
}
