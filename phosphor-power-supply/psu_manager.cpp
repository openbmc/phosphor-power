...
void PSUManager::createErrorPEL(
    ...
    std::map<std::string, std::string> additionalData)
{
    ...
    {
        log<level::ERR>(e.what(), entry("PATH=%s", loggingObjectPath));
    }
}
...
void PSUManager::analyze()
{
    ...
    for (auto& psu : psus)
    {
        ...
        if (!psu->isFaultLogged() && psu->isFaulted())
        {
            ...
            additionalData["STATUS_WORD"] = std::to_string(psu->getStatusWord());
            ...
            if ((psu->hasInputFault() || psu->hasVINUVFault()))
            {
                ...
                createErrorPEL("xyz.openbmc_project.Power.PowerSupply."
                               "Error.InputFault",
                               additionalData);
                psu->setFaultLogged();
            }
