...
class Chassis
{
  ...
  private:
    ...
    /**
     * @brief Get object path unique ID. Unique ID is the slot number of the
     * object. Invalid object path unique ID is defined as 9999
     *
     * @param[in] D-Bus service name (Entity Manager or Inventory Manager)
     * @param[in] Object Path
     * @return uint64_t Object path uinique id (currently the object slot
     * number) or invalidObjecPathId (9999).
     */
    uint64_t getObjectPathUniqueId(const std::string& service,
                                   const std::string& objectPath);

    /**
     * @brief Extracts backplane path up to and including "Backplane". This
     * function searchs for the substring "Backplane" within a path. Once found,
     * it returns the path from the begining up to the full path segment that
     * contains "Backplane"
     *
     * @param[in] Power supply path
     * @return string - Return a substring of powersupply path or empty if
     * Backplane not found.
     */
    std::string extractBackplanePath(const std::string& path);
};
