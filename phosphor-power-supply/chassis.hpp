...
class Chassis
{
  ...
  private:
    ...
    /**
     * @brief The Chassis ID is the last digit in the chassis path.
     */
    char chassisId;
    ...
    /**
     * @brief Helper function to populate the chassis properties
     *
     * @param[in] properties - A map of property names and values
     */
    void populateChassisProperties(const util::DbusPropertyMap& properties);
    ...
    /**
     * @brief Checks if the given object string contains a substring that
     *        matches the current chassis. The current chassis is formed
     *        by concatinating 'chassis' with the chassis ID.
     *
     * @param[in] The chassis object path
     * @return True or False
     */
    bool matchesThisChassis(const auto& objectPath)
    {
        std::string chassis = "chassis";
        std::string token;
        chassis += chassisId;

        std::istringstream stream(objectPath);
