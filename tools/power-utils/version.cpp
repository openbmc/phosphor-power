...
bool checkFileExists(const std::string& filePath)
{
    ...
    {
        log<level::ERR>("Unknown error occured while checking file existence");
    }
    return false;
}
} // namespace utils
                       ...
                       const std::string& psuInventoryPath)
{
    try
    {
        ...
        std::string fwVersion = utils::readVPDValue(*pmbusIntf, FW_VERSION,
    ...
    {
        log<level::ERR>(std::format("Runtime error: {}", e.what()).c_str());
        return {};
    }
    catch (const std::exception& e)
    {
