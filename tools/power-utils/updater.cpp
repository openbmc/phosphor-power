    ...
    devName(internal::getDeviceName(devPath)), imageDir(imageDir)
{
    ...
    driverPath = fs::canonical(p); // Get the path that points to the driver dir
    bindUnbind(false);
}
...
// errors. So set the PSU inventory's Presnet proerty to false so that
...
void Updater::setPresent(bool present)
{
    ...
    util::setProperty(INVENTORY_IFACE, PRESENT_PROP, psuInventoryPath, service,
                      bus, present);
}
