...
bool AeiUpdater::downloadPsuFirmware()
{
    ...
    while ((bytesRead < fileSize) && !downloadFailed)
    {
        ...
        downloadFailed = !performI2cWriteReadWithRetries(
            ISP_MEMORY_REGISTER, cmdBlockWrite, EXPECTED_MEM_READ_REPLY,
            readData, MAX_RETRIES, writeBlockDelay);
    ...
    const int delayTime)
{
    for (int i = 0; i < retries; ++i)
    {
        ...
        try
        {
            performI2cWriteRead(regAddr, cmdBlockWrite, readReplySize, readData,
                                delayTime);
...
void AeiUpdater::performI2cWriteRead(
    uint8_t regAddr, const std::vector<uint8_t>& cmdBlockWrite,
    uint8_t& readReplySize, uint8_t* readData, const int& delayTime)
{
