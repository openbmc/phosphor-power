...
class PldmFetcher
{
  ...
  private:
    ...
    /**
     * FRU location codes keyed by entityInstance.
     ...
     * Derived by matching fruLocationCache values against Inventory location
     * codes.
     */
    ...
    /**
     * Finds the OEM location code field in raw FRU table data.
     *
     * @param[in] fruBytes  Raw FRU table bytes.
     *
     * @return Location code string, or std::nullopt if not found.
     */
    std::optional<std::string> findFruLocationCode(
        const std::vector<uint8_t>& fruBytes) const;

    /**
     * Fetches the OEM location code for a chassis via a targeted MCTP
     * GetFRURecordByOption request.
     *
     * @param[in] rsi  FRU Record Set Identifier to query.
     *
     * @return Location code string, or std::nullopt on failure or if not found.
     */
    std::optional<std::string> readPldmLocationCode(uint16_t rsi) const;

    /**
     * Reads the LocationCode property from the Inventory Manager for the given
     * chassis number.
     *
     * @param[in] chassisNumber  Chassis index (0-based).
     *
     * @return Location code string, or std::nullopt if not found or on error.
     */
    std::optional<std::string> readInventoryLocationCode(
