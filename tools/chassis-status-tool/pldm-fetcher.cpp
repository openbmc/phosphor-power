...
PldmFetcher::PldmFetcher(sdbusplus::bus_t& bus, bool isVerbose, int numChassis,
                         uint8_t mctpEid) :
    verbose(isVerbose), numChassis(numChassis), bus(bus),
    tid(static_cast<pldm_tid_t>(mctpEid))
{
    ...
    pdrSets.availability.logical =
        fetchSensorPDRs(PLDM_STATE_SET_AVAILABILITY, logicalChassisEntityType);
    pdrSets.presence.physical =
        fetchSensorPDRs(PLDM_STATE_SET_PRESENCE, chassisEntityType);
    pdrSets.presence.logical =
        fetchSensorPDRs(PLDM_STATE_SET_PRESENCE, logicalChassisEntityType);
    pdrSets.powerState.physical =
        fetchSensorPDRs(PLDM_STATE_SET_SYSTEM_POWER_STATE, chassisEntityType);
    pdrSets.powerState.logical = fetchSensorPDRs(
        PLDM_STATE_SET_SYSTEM_POWER_STATE, logicalChassisEntityType);
    ...
    pdrSets.operationalFault.logical = fetchSensorPDRs(
        PLDM_STATE_SET_OPERATIONAL_FAULT_STATUS, logicalChassisEntityType);
    ...
    pldm_transport_mctp_demux_map_tid(demux,
                                      static_cast<pldm_tid_t>(mctpEid),
                                      mctpEid);
