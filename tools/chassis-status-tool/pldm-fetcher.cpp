...
PldmFetcher::PldmFetcher(sdbusplus::bus_t& bus, bool isVerbose, int numChassis,
                         uint8_t mctpEid) :
    verbose(isVerbose), bus(bus), tid(static_cast<pldm_tid_t>(mctpEid))
{
    // Fetch all PDRs up front via D-Bus (physical chassis only)
    ...
    pdrSets.powerState =
        fetchSensorPDRs(PLDM_STATE_SET_SYSTEM_POWER_STATE, chassisEntityType);
    ...
    {
        auto locCode = readPldmLocationCode(entry.rsi);
        if (locCode)
        {
    ...
    // Match PLDM FRU location codes against D-Bus Inventory location codes to
    // map chassisNumber → entityInstance.
    for (const auto& [entityInstance, pldmLocCode] : fruLocationCache)
    {
        for (int i = 0; i <= numChassis; ++i)
        {
            auto invLocCode = readInventoryLocationCode(i);
        ...
        pldm_instance_id_t instanceId, uint16_t rsi) const
{
    ...
    const auto rc = encode_get_fru_record_by_option_req(
        ...
        PLDM_FRU_RECORD_TYPE_OEM,              // recordType
        PLDM_OEM_FRU_FIELD_TYPE_LOCATION_CODE, // fieldType
    ...
    {
        std::println(stderr,
                     "{}readPldmLocationCode: encode request failed for "
                     "rsi={}: rc={}",
                     largeIndent, rsi, rc);
    }
        ...
        uint16_t rsi, void* responseMsg, size_t responseSize) const
{
    ...
    if (rc != PLDM_SUCCESS || completionCode != PLDM_SUCCESS)
    {
        ...
        {
            std::println(stderr,
                         "{}readPldmLocationCode: decode failed for rsi={}: "
                         "rc={} cc={}",
                         largeIndent, rsi, rc,
                         static_cast<unsigned>(completionCode));
        }
...
std::optional<std::string> PldmFetcher::findFruLocationCode(
    const std::vector<uint8_t>& fruBytes) const
{
        ...
        end)
    {
        ...
        for (uint8_t field = 0; field < rec->num_fru_fields; ++field)
        {
            ...
            if (rec->record_type == PLDM_FRU_RECORD_TYPE_OEM &&
                tlv->type == PLDM_OEM_FRU_FIELD_TYPE_LOCATION_CODE)
            {
...
std::optional<std::string> PldmFetcher::readPldmLocationCode(uint16_t rsi) const
{
    const auto instanceId = allocateInstanceId("readPldmLocationCode");
    if (!instanceId)
    {
        return std::nullopt;
    }
    ...
    if (rc != PLDM_REQUESTER_SUCCESS)
    {
        ...
        {
            std::println(stderr,
                         "{}readPldmLocationCode: MCTP send/recv failed "
                         "for rsi={}: rc={}",
    ...
    return findFruLocationCode(*fruBytes);
}

auto PldmFetcher::readInventoryLocationCode(int chassisNumber) const
    -> std::optional<std::string>
{
    try
    {
        ...
        method.append("xyz.openbmc_project.Inventory.Decorator.LocationCode",
                      "LocationCode");
...
void PldmFetcher::display(int chassisNumber) const
{
    ...
    printSensorState(findSensorEntry(pdrSets.operationalFault, entityInstance),
    ...
    auto invOpt = readInventoryLocationCode(chassisNumber);
    ...
    std::println("{}Location Code (PLDM):      {}", largeIndent, pldmLocCode);
    std::println("{}Location Code (Inventory): {}", largeIndent, invLocCode);
}
