...
TEST(ConfigFileParserTests, Parse)
{
    ...
    // Test where works: Chassis with all optional properties
    {
        ...
        EXPECT_EQ(chassis[0]->getNumber(), 1);
        EXPECT_EQ(chassis[0]->getDevices().size(), 5);
    }
...
TEST(ConfigFileParserTests, ParseChassis)
{
    ...
    // Test where works: All optional GPIO properties specified
    {
        ...
        EXPECT_EQ(chassis->getPresencePath(),
                  "/xyz/openbmc_project/inventory/dev/i2c-259");
        EXPECT_EQ(chassis->getDevices().size(), 5);
        EXPECT_EQ(chassis->getDevices()[0]->getName(), "presence-chassis2");
        EXPECT_EQ(chassis->getDevices()[1]->getName(),
                  "power-chassis2-standby-fault-unlatched");
        EXPECT_EQ(chassis->getDevices()[2]->getName(),
                  "power-chassis2-standby-fault-latched");
        EXPECT_EQ(chassis->getDevices()[3]->getName(),
                  "power-chassis2-standby-fault-reset");
        EXPECT_EQ(chassis->getDevices()[4]->getName(),
    ...
    // Test where works: Only PresenceGpio specified
    {
        ...
        EXPECT_EQ(chassis->getDevices()[0]->getName(), "presence-chassis3");
    }
...
TEST(ConfigFileParserTests, ParseDevice)
{
    // Test where works: All required properties specified
    {
        ...
        EXPECT_EQ(device->getName(), "presence-chassis1");
        EXPECT_EQ(device->getDirection(), "Input");
        EXPECT_EQ(device->getPolarity(), "Low");
    }
    ...
    // Test where works: Output direction and High polarity
    {
        ...
        std::unique_ptr<Device> device = parseDevice(element);
        EXPECT_EQ(device->getName(), "power-chassis1-standby-fault-reset");
        EXPECT_EQ(device->getDirection(), "Output");
        EXPECT_EQ(device->getPolarity(), "High");
    }
...
TEST(ConfigFileParserTests, ParseManyChassis)
{
    // Test where works: Chassis with all optional properties
    {
        ...
        std::vector<int> devicesPerChassis = {1, 2, 3, 4, 5, 4, 3, 2, 1, 0};

        std::vector<std::string> devicesPresenceGpio = {
        ...
        std::vector<std::string> devicesFaultUnlatchedGpio = {
        ...
        std::vector<std::string> devicesFaultLatchedGpio = {
        ...
        std::vector<std::string> devicesFaultLatchResetGpio = {
        ...
        std::vector<std::string> devicesEnableSystemResetGpio = {
        ...
        std::vector<std::string> devicesDirection = {
            "Input",  "Input", "Input", "Output", "Output",
            "Output", "Input", "Input", "Input",  "",
        };

        std::vector<std::string> devicesPolarity = {
        ...
        for (auto& thisChassis : chassis)
        {
            ...
            EXPECT_EQ(thisChassis->getDevices().size(),
                      devicesPerChassis[indexNumber]);
            const auto& chassisDevices = thisChassis->getDevices();
            EXPECT_EQ(chassisDevices.size(), devicesPerChassis[indexNumber]);
            int thisDevicesIndex = 0;
            // Loop over this chassis's vector of devices.
            for (auto& thisDevices : chassisDevices)
            {
                switch (thisDevicesIndex)
                {
                    case 4:
                        EXPECT_EQ(thisDevices->getName(),
                                  devicesEnableSystemResetGpio[indexNumber]);
                        break;
                    case 3:
                        EXPECT_EQ(thisDevices->getName(),
                                  devicesFaultLatchResetGpio[indexNumber]);
                        break;
                    case 2:
                        EXPECT_EQ(thisDevices->getName(),
                                  devicesFaultLatchedGpio[indexNumber]);
                        break;
                    case 1:
                        EXPECT_EQ(thisDevices->getName(),
                                  devicesFaultUnlatchedGpio[indexNumber]);
                        break;
                    case 0:
                        EXPECT_EQ(thisDevices->getName(),
                                  devicesPresenceGpio[indexNumber]);
                        break;
                    default:
                        break;
                }
                EXPECT_EQ(thisDevices->getDirection(),
                          devicesDirection[thisDevicesIndex]);
                EXPECT_EQ(thisDevices->getPolarity(),
                          devicesPolarity[thisDevicesIndex]);
                ++thisDevicesIndex;
            }
            ++indexNumber;
        }
