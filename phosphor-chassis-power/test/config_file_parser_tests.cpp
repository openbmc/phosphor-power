...
TEST_F(ConfigFileParserTests, Parse)
{
...
TEST_F(ConfigFileParserTests, ParseChassis)
{
...
TEST_F(ConfigFileParserTests, ParseChassisArray)
{
    // Test where works
    {
        const json element = R"(
...
TEST_F(ConfigFileParserTests, ParseGPIOs)
{
    // Test where works: All required properties specified (no default)
    {
        ...
        std::unique_ptr<GpioInterface> gpio = parseGpio(element, services);
    ...
    // Test where works: Input direction, and Default property with Low value
    {
        const json element = R"(
            {
              "Name": "presence-chassis1",
    ...
    // Test where works: Input direction, Default property with High value
    {
        const json element = R"(
            {
    ...
    // Test where fails: Output direction, Default property with High value
    try
    {
        const json element = R"(
            {
    ...
    {
        EXPECT_STREQ(e.what(), "default value not valid with output direction");
    }
    ...
    // Test where fails: Default value is not a 0 or 1, it is greater
    try
    {
        const json element = R"(
            {
    ...
    // Test where fails: Default value is not a 0 or 1, it is lesser
    try
    {
        const json element = R"(
            {
    ...
    // Test where fails: Default value is invalid string bool true
    try
    {
        const json element = R"(
            {
    ...
    // Test where fails: Default value is invalid string bool false.
    try
    {
        const json element = R"(
            {
...
TEST_F(ConfigFileParserTests, parsePresencePath)
{
    // Test where works: Path with leading slash
    {
        const json element = R"( "/dev/i2c-159" )"_json;
        std::string path = parsePresencePath(element);
        EXPECT_EQ(path, "/dev/i2c-159");
    }

    // Test where fails: Relative path (no leading slash)
    try
    {
        const json element = R"( "dev/i2c-159" )"_json;
        std::string path = parsePresencePath(element);
    ...
    {
        EXPECT_STREQ(e.what(), "Invalid absolute path");
    }
...
TEST_F(ConfigFileParserTests, ParseRoot)
{
    ...
    // Test where works: multi Chassis with all optional properties
    {
        const json configFileContents = R"(
            [
              {
