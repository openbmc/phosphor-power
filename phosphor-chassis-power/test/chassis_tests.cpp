...
TEST_F(ChassisTests, getGpios)
{
    ...
    // Test where GPIOs were specified in constructor (without default values)
    {
        ...
        EXPECT_FALSE(chassisGpios[0]->getDefault().has_value());
        ...
        EXPECT_FALSE(chassisGpios[1]->getDefault().has_value());
        ...
        EXPECT_FALSE(chassisGpios[2]->getDefault().has_value());
    }
    ...
    // Test where GPIOs with default values were specified in constructor
    {
        ...
        gpios.emplace_back(
            std::make_unique<Gpio>("presence-chassis1", GpioDirection::Input,
                                   GpioPolarity::Low, GpioPolarity::Low));

        gpios.emplace_back(std::make_unique<Gpio>(
            "power-fault-unlatched", GpioDirection::Input, GpioPolarity::Low,
            GpioPolarity::High));
        ...
        EXPECT_TRUE(chassisGpios[0]->getDefault().has_value());
        EXPECT_EQ(chassisGpios[0]->getDefault().value(), GpioPolarity::Low);

        // Verify GPIO with default High
        EXPECT_EQ(chassisGpios[1]->getName(), "power-fault-unlatched");
        EXPECT_EQ(chassisGpios[1]->getDirection(), GpioDirection::Input);
        EXPECT_EQ(chassisGpios[1]->getPolarity(), GpioPolarity::Low);
        EXPECT_TRUE(chassisGpios[1]->getDefault().has_value());
        EXPECT_EQ(chassisGpios[1]->getDefault().value(), GpioPolarity::High);
