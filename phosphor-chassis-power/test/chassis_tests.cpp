...
TEST_F(ChassisTests, getGpios)
{
    ...
    // Test where GPIOs were specified in constructor
    {
        ...
        gpios.emplace_back(std::make_unique<Gpio>(
            "DeviceName_1", parseDirection("Input"), parsePolarity("High")));

        gpios.emplace_back(std::make_unique<Gpio>(
            "DeviceName_2", parseDirection("Input"), parsePolarity("Low")));

        gpios.emplace_back(std::make_unique<Gpio>(
            "DeviceName_3", parseDirection("Output"), parsePolarity("High")));
