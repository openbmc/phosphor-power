...
TEST_F(ChassisTests, HandleBMCReset_PresentSled_ChassisOn)
{
    // Test Present sled, no faults, D-Bus reports chassis ON:
    {
        ...
        expectSbFaultLatched(chassis, 0);   // set return disabled
...
TEST_F(ChassisTests, HandleBMCReset_60SecTimer_NulloptPowerStatus)
{
    ...
    //       is started:
    {
        ...
        expectSbFaultUnLatched_read2(chassis, 0, 0); // set disabled, disabled

        expectSbFaultLatched_read2(chassis, 0, 0);   // set disabled, disabled
...
{
    // Test Present sled, FAULT after power on - power-chs1-sb-fault-unlatched.
    {
        MockServices services{};
        Chassis chassis = buildSledChassis(services, event);
...
TEST_F(ChassisTests, handleBMCResetTimerCallback)
{
    ...
    {
        // SHELDON:QUESTION: this test case seems to have missed the boot Poff
        MockServices services{};
        Chassis chassis = buildSledChassis(services, event);
        ...
        EXPECT_EQ(chassis.getState(), ChassisState::Off); // SHELDON:DEBUG:FAULT
