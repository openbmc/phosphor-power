...
TEST_F(ChassisTests, getPowerSystemInputsInterface)
{
    ...
    // Test where interface has been set to Good
    {
        ...
        // Create interface and set to Good
        auto interface = std::make_unique<ChassisPowerSystemInterface>(
            bus, "/xyz/openbmc_project/state/chassis1",
            PowerSystemInputs::Status::Fault, true);
        chassis.setPowerSystemInputsInterface(bus);
...
TEST_F(ChassisTests, setPowerSystemInputsInterface)
{
    // Test setting interface successfully
    {
        ...
        bool result = chassis.setPowerSystemInputsInterface(bus);
