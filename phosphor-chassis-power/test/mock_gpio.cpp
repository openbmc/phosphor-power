...
class MockGpio : public GpioInterface
{
  public:
    ...
    {
        // Set up default behavior for property getters only
        // These return const values that don't change during test execution
...
{
    return std::make_unique<MockGpio>(name, direction, polarity, defaultValue);
}
} // namespace phosphor::power::chassis
