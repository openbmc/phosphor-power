...
class Chassis
{
  ...
  private:
     ...
     *
     * Writes the fault reset GPIO and logs the PEL. Returns true on success.
     * Returns false if the GPIO write failed and the caller should retry.
     */
    bool handleLatchedFault();

    /**
     * Writes a value to a GPIO by name and releases the line.
     *
     * Write to the GPIO with the given value, then releases the line.
     *
     * @param name Substring to identify the GPIO by name
     * @param value Value to write
     *
     * @return true on success, false if the write failed
     */
    bool writeAndReleaseGPIO(std::string_view name, int value);
