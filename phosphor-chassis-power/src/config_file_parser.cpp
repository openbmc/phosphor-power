...
std::unique_ptr<Device> parseDevice(const json& element)
{
    ...
    std::string direction = parseString(directionElement);
    ++propertyCount;

    // Required Polarity property
    const json& polarityElement = getRequiredProperty(element, "Polarity");
    std::string polarity = parseString(polarityElement);
    ++propertyCount;

    // Verify no invalid properties exist
    verifyPropertyCount(element, propertyCount);

    return std::make_unique<Device>(name, direction, polarity);
}
