...
bool Chassis::hasPowerFault(const std::string& gpioNamePattern)
{
    ...
    Gpio* gpioCheckObj = getGpioByName(gpioNamePattern);
    if (gpioCheckObj != nullptr)
    {
        try
        {
            if (!gpioCheckObj->foundLine())
            {
                gpioCheckObj->findLine();
            }
            if (gpioCheckObj->foundLine() && gpioCheckObj->requestRead())
            {
                int value = gpioCheckObj->getValue();
