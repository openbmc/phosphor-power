...
bool Chassis::initializeStatusMonitor(sdbusplus::bus_t& bus)
{
    try
    {
        ...
        // Set up D-Bus subscription for power state changes//SHELDON::BOB1
        // start
...
std::optional<bool> Chassis::isChassisPoweredOn() const
{
    ...
    try
    {
        return statusMonitor->isPoweredOn(); // SHELDON:?TODO: change this to do
                                             // read, and remove Monitor, and if
                                             // this reads enable callback!
    }
    catch (const std::exception& e)
    {
        ...
        else if (name.contains(faultUnlatchedName))
        {
            if (gpio->requestRead())
            {
                ...
                if (changed)
                {
                    ...
                    // if (gpio->getValue() == 1) //SHELDON:READ 2
                    if (faultUnlatchedValue == 1)
                    {
                        handlePowerFault(hasPowerFault("fault-latched"));
                    }
                    // SHELDON:QUESTION: I think I need to have an else, to
                    //          check latched, and if latched write GPIO to
                    //          clear.
                }
                gpio->release(); // SHELDON:DEBUG:remove
            }
...
bool Chassis::gpioValueChanged(Gpio& gpio, std::optional<int>& gpioValue)
{
    ...
    value = gpio.getValue(); // SHELDON:READ 1
...
bool Chassis::hasPowerFault(const std::string& gpioNamePattern)
{
    ...
    if (faultUnlatchedGpio != nullptr)
    {
        try
        {
            ...
            {
                int value = faultUnlatchedGpio->getValue(); // SHELDON:FAIL:????
                ...
                if (gpioNamePattern == "fault-unlatched")
                {
                    ...
                    {
                        lg2::error(
                            "----------------- power-chassis{CHASSIS}-standby-fault-unlatched:Sx_STBY_PG:{VALUE} -> ACTIVE -> BAD_SB_POWER",
                            "CHASSIS", number, "VALUE", value);
                        lg2::error(
                            "                      This signal is used to tell the live state of the sled. Does it have standby power right now?");
                        lg2::error(
                            "                      0 = vin bad, 1 = vin good");
                        return true;
                    }
                    lg2::error(
                        "----------------- power-chassis{CHASSIS}-standby-fault-unlatched:Sx_STBY_PG:{VALUE} -> not active -> GOOD_SB_POWER",
                        "CHASSIS", number, "VALUE", value);
                    lg2::error(
                        "                      This signal is used to tell the live state of the sled. Does it have standby power right now?");
                    lg2::error(
                ...
                else
                {
                    ...
                    {
                        lg2::error(
                            "----------------- power-chassis{CHASSIS}-standby-fault-latched:Sx_STBY_PG_FAULT_LATCHED_N:{VALUE} -> ACTIVE -> SB POWER_BAD",
                            "CHASSIS", number, "VALUE", value);
                        lg2::error(
                            "                      This signal tells the BMC which sled lost standby power.");
                        lg2::error(
                            "                      It is used so that we can log an error (741x) after the fact against the sled that caused the reset.");
                        lg2::error(
                            "                      0 = vin bad, 1 = vin good");
                        return true;
                    }
                    lg2::error(
                        "----------------- power-chassis{CHASSIS}-standby-fault-latched:Sx_STBY_PG_FAULT_LATCHED_N:{VALUE} -> not active -> SB POWER_GOOD",
                        "CHASSIS", number, "VALUE", value);
                    lg2::error(
                        "                      This signal tells the BMC which sled lost standby power.");
                    lg2::error(
...
void Chassis::handleBMCReset()
{
    ...
    else
    {
        ...
        updatePowerSystemInputsStatus(
            false); // SHELDON:QUESTION: where did this come from ? VALIDATE
    }
}
...
void Chassis::handlePowerFault(bool stdbyPowerFault)
{
    ...
    lg2::error(
        "chassis{CHASSIS} power fault detected {STRING} loss of standby power",
