...
std::optional<bool> Chassis::isChassisPoweredOn() const // SHELDON:PLACE:1
{
    if (!statusMonitor)
    {
        ...
        else if (name.contains(faultUnlatchedName))
        {
            if (gpio->requestRead())
            {
                ...
                if (changed)
                {
                    ...
                    // SHELDON:DEBUG: setPowerSystemInputsStatus vs
...
bool Chassis::hasPowerFault(const std::string& gpioNamePattern)
{
    ...
    // SHELDON:TODO: change name to make generic
    ...
    if (faultUnlatchedGpio != nullptr)
    {
        try
        {
                ...
                faultUnlatchedGpio->requestRead())
            {
                ...
                else if (gpioNamePattern == "fault-latched")
                {
                    // Active (1) means fault
                    if (value == 1)
                    {
