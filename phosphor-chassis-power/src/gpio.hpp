...
class GpioInterface
{
  public:
     ...
     */
    virtual bool getPreviousValue() = 0;
    /**
...
class Gpio : public GpioInterface
{
  public:
     ...
     * @param initialValue optional initial value for the previous value
     *                     tracking; does not set the actual GPIO value
     */
    explicit Gpio(const std::string& name, GpioDirection direction,
                  GpioPolarity polarity,
                  std::optional<bool> initialValue = std::nullopt);
    ...
    /** @copydoc GpioInterface::getValue() */
    virtual bool getPreviousValue() override;
 ...
 * @param initialValue optional initial value for the previous value
 *                     tracking; does not set the actual GPIO value.
 *                     If not provided, defaults to nullopt.
