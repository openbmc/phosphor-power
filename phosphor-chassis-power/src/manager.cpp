    ...
    services(services)
{
    ...
    blackoutTargetMatch = std::make_unique<sdbusplus::match>(
        services.getBus(),
        ...
        std::bind_front(&Manager::blackoutTargetStarted, this));
...
void Manager::blackoutTargetStarted(sdbusplus::message_t& msg)
{
    uint32_t id{};
    ...
    if (unit == blackoutTarget)
    {
        ...
        {
            // Config file not yet loaded; defer until loadConfigFile() runs
