...
If the PresenceGpio can be detected, and indicate true or false (0 or 1) then
the chassis is considered present. if the PresenceGpio is not detected/false,
then if the PresencePath is present/exists the chassis is present. If both
PresenceGpio is not detected/false and PresencePath is not present/exists, the
