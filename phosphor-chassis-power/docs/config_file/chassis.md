...
If the PresenceGpio can be detected, and indicate true or false (0 or 1). if
PresenceGpio is detected/true then the chassis is considered present. if the
PresenceGpio is not detected/false then the this looks at the PresencePath. if
the PresencePath is present/exists, then this is treated as the chassis is
present. If both PresenceGpio is not detected/false and PresencePath is not
present/exists, the chassis is not present.
...
Each chassis can have multiple GPIO entries with the format outlined
[GPIO format](gpio.md).
