...
std::unique_ptr<I2CWriteBitAction> parseI2CWriteBit(const json& element)
{
    ...
    const json& registerElement = getRequiredProperty(element, "register");
    uint8_t registerValue = parseStringToUint8(registerElement);
    ++propertyCount;

    // Required position property
    const json& positionElement = getRequiredProperty(element, "position");
    uint8_t positionValue = parseUint8(positionElement);
    ++propertyCount;

    // Required value property
    const json& valueElement = getRequiredProperty(element, "value");
    uint8_t valueValue = parseUint8(valueElement);
    ++propertyCount;

    // Verify no invalid properties exist
    verifyPropertyCount(element, propertyCount);

    return std::make_unique<I2CWriteBitAction>(registerValue, positionValue,
                                               valueValue);
}
