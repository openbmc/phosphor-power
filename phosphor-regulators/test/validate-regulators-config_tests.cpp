                                ...
                                std::string& standardError)
{
    ...
    tmpFile newTmpFile;
    std::string errorTmpFile = newTmpFile.getName();
...
void writeDataToTmpFile(const json configFileJson, std::string fileName)
{
...
void expectJsonValid(const json configFileJson)
{
    std::string fileName;
    tmpFile newTmpFile;
    fileName = newTmpFile.getName();
    writeDataToTmpFile(configFileJson, fileName);
...
TEST(ValidateRegulatorsConfigTest, CommandLineSyntax)
{
    ...
    tmpFile newTmpFile;
    fileName = newTmpFile.getName();
    writeDataToTmpFile(validConfigFile, fileName);
    // Valid: -s specified
    {
    ...
    // Invalid: File specified after -s is not right data format
    {
        ...
        tmpFile newTmpFile;
        wrongFormatFile = newTmpFile.getName();
    ...
    // Invalid: File specified after -c is not readable
    {
        ...
        tmpFile newTmpFile;
        notReadableFile = newTmpFile.getName();
        writeDataToTmpFile(validConfigFile, notReadableFile);
        command = validateTool + schema + schemaFile + configuration +
                  notReadableFile;
