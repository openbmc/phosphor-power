...
/*
 * Tests for the ConfigFileParser class
 */
TEST(ConfigFileParserTests, Constructor)
{
    // Test where works
    {
        ...
        TmpFile configFile;
        std::ofstream configFileStream{configFile.getName()};
        configFileStream << configFileContents;
        configFileStream.close();
        std::filesystem::path pathName{configFile.getName()};
        ConfigFileParser parser{pathName};
        // TODO: Not implemented yet
        // EXPECT_EQ(parser.getChassis().size(), 2);
        // EXPECT_EQ(parser.getChassis()[0]->getNumber(), 1);
        // EXPECT_EQ(parser.getChassis()[1]->getNumber(), 3);
        EXPECT_EQ(parser.getPathName(), pathName);
        EXPECT_EQ(parser.getRules().size(), 1);
        EXPECT_EQ(parser.getRules()[0]->getID(), "set_voltage_rule");
    }
    ...
    try
    {
        ...
        configFileStream << configFileContents;
        configFileStream.close();
        chmod(configFile.getName().c_str(), 0222);
        std::filesystem::path pathName{configFile.getName()};
        ConfigFileParser parser{pathName};
        ADD_FAILURE() << "Should not have reached this line.";
    }
...
TEST(ConfigFileParserTests, GetChassis)
{
    // TODO: Not implemented yet
}

TEST(ConfigFileParserTests, GetPathName)
{
    const json configFileContents = R"(
        {
          "chassis": [ { "number": 1 } ]
...
TEST(ConfigFileParserTests, GetRules)
{
    const json configFileContents = R"(
        {
          "rules": [
            ...
            },
            {
              ...
              "actions": [
                { "pmbus_write_vout_command": { "volts": 1.04, "format": "linear" } }
              ]
            }
          ],
          "chassis": [
    ...
    std::filesystem::path pathName{configFile.getName()};
    ConfigFileParser parser{pathName};
    EXPECT_EQ(parser.getRules().size(), 2);
    EXPECT_EQ(parser.getRules()[0]->getID(), "set_voltage_rule1");
    EXPECT_EQ(parser.getRules()[1]->getID(), "set_voltage_rule2");
}

/*
 * Tests for the config_file_parser::internal utility functions
 */
...
TEST(ConfigFileParserTests, ParseRoot)
{
    // Test where works: Only required properties specified
    {
        ...
        parseRoot(element, rules, chassis);
