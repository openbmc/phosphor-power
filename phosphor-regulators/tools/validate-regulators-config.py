    ...
    if 'rules' in config_json:
        for rule in config_json['rules']:
            ...
            if rule_id in rule_ids:
                ...
                Found multiple rules with the ID ".format(rule['id'])+'\n')
    ...
    if 'chassis' in config_json:
        for chassis in config_json['chassis']:
            ...
            if number in numbers:
                ...
                Found multiple chassis with the number ".format(chassis['number'])+'\n')
                handle_validation_error()
            else:
            ...
            if 'devices' in chassis:
                for device in chassis['devices']:
                    ...
                    if deivce_id in device_ids:
                        ...
                        Found multiple devices with the ID ".format(device['id'])+'\n')
                        handle_validation_error()
                    else:
                    ...
                    if 'rails' in device:
                        for rail in device['rails']:
                            ...
                            if rail_id in rail_ids:
                                ...
                                Found multiple rails with the ID ".format(rail['id'])+'\n')
                                handle_validation_error()
                            else:
