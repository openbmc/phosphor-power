...
def get_values(json_element, key, result = None):
    r"""
    Finds all occurrences of a key within the config file and returns the
    associated values.
...
def check_number_of_elements_in_masks(config_json):
    ...
    for index in i2c_write_bytes:
        if 'masks' in index:
            if len(index.get('masks', [])) != len(index.get('values', [])):
                sys.stderr.write("Error: Invalid i2c_write_bytes action.\n"+\
                "The masks array must have the same size as the values array. "+\
                "masks: "+str(index.get('masks', []))+\
                ", values: "+str(index.get('values', []))+'.\n')
                handle_validation_error()

    for index in i2c_compare_bytes:
        if 'masks' in index:
            if len(index.get('masks', [])) != len(index.get('values', [])):
...
def check_rule_id_exists(config_json):
    ...
    valide_rule_id = get_rule_ids(config_json)
    for rule_id in rule_ids:
        if rule_id not in valide_rule_id:
...
def check_run_rule_value_exists(config_json):
    ...
    run_rule = get_values(config_json, 'run_rule')
    valid_rule_ids = get_rule_ids(config_json)
    for rule_ids in run_rule:
        if rule_ids not in valid_rule_ids:
            sys.stderr.write("Error: Rule ID does not exist.\n"+\
