    ...
    for object in i2c_write_bytes + i2c_compare_bytes:
        if "masks" in object:
            ...
            if len(masks) != len(values):
                action = "i2c_write_bytes" if object in i2c_write_bytes else "i2c_compare_bytes"
                raise ValidationError(
...
def parse_arguments():
    ...
    try:
        ...
        if not is_json_file(args.schema_file):
            raise CommandLineError("Error: Schema file is not in the JSON format.")
        ...
        if not os.access(args.configuration_file, os.R_OK):
            raise CommandLineError("Error: Configuration file is not readable.")
        if not is_json_file(args.configuration_file):
            raise CommandLineError("Error: Configuration file is not in the JSON format.")
