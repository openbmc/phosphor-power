...
If an error occurs while executing actions, the error will be logged.  Any
remaining actions for the current Device/Rail will be skipped.  However,
...
If an error occurs while reading the sensors for a Rail, the error will be
logged.  The value of all D-Bus sensors for this Rail will be set to 0.  The
Functional property of all D-Bus sensors for this Rail will be set to false.
Sensor monitoring will still be performed for all remaining Rail objects in the
system.  If the same error occurs repeatedly on a Rail, it will only be logged
