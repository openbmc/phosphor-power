    ...
    sd_bus_error* error)
{
    if (msg != nullptr && context != nullptr)
    {
        try
        {
            ...
            lg2::info("callbackGetPgood: {PGOOD}", "PGOOD", pgood);
    ...
    sd_bus_error* error)
{
    if (msg != nullptr && context != nullptr)
    {
        try
        {
            ...
            lg2::info("callbackGetPgoodTimeout: {TIMEOUT}", "TIMEOUT", timeout);
                                          ...
                                          sd_bus_error* error)
{
    if (msg != nullptr && context != nullptr)
    {
        try
        {
            ...
            int pgood = pwrObj->getPgood();
            lg2::info("callbackGetPowerState: {PGOOD}", "PGOOD", pgood);
    ...
    sd_bus_error* error)
{
    if (msg != nullptr && context != nullptr)
    {
        try
        {
            ...
            lg2::info("callbackGetState: {STATE}", "STATE", state);
