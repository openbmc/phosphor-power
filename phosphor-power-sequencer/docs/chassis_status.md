...
`Fault`, it means that the chassis is experiencing a
...
- The chassis power good signal from the power sequencer will be read. The
  `pgood` property on the `org.openbmc.control.Power` interface for the chassis
  will be set to the value of this signal.
- The chassis will be powered on when the system is being powered on. The
  brownout could be very brief, in which case the power on attempt may succeed.
  The `state` property on the `org.openbmc.control.Power` interface for the
  chassis will be set to 1. If the brownout continues, the chassis power on
  attempt will likely fail due to a chassis pgood fault. See
  [Power Good Faults](pgood_faults.md) for more information on how the error
  will be handled.
