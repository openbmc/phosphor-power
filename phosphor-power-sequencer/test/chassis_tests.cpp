...
TEST(ChassisTests, ToString)
{
    EXPECT_EQ(Chassis::toString(Chassis::UnexpectedStatusType::alreadyAtState),
              "Chassis is already at requested state");

    EXPECT_EQ(Chassis::toString(Chassis::UnexpectedStatusType::notPresent),
              "Chassis is not present");

    EXPECT_EQ(Chassis::toString(Chassis::UnexpectedStatusType::notEnabled),
              "Chassis is not enabled");

    EXPECT_EQ(Chassis::toString(Chassis::UnexpectedStatusType::noInputPower),
              "Chassis does not have input power");

    EXPECT_EQ(Chassis::toString(Chassis::UnexpectedStatusType::notAvailable),
...
TEST(ChassisTests, Constructor)
{
    ...
    EXPECT_EQ(chassis.getObjectPath(), "/org/openbmc/control/power1");
    EXPECT_FALSE(chassis.hasDBusInterface());
    EXPECT_THROW(chassis.getDBusInterface(), std::runtime_error);
}
...
TEST(ChassisTests, CanSetPowerState)
{
    ...
    try
    {
        ...
        chassis->canSetPowerState(PowerState::on, services);
        ADD_FAILURE() << "Should not have reached this line.";
    }
    catch (const std::runtime_error& e)
    {
    ...
    // Test where false: Chassis is not present and request is on
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
    ...
    // Test where true: Chassis is not present, but request is off
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
        ...
        EXPECT_CALL(monitor, isPresent)
            ...
            .WillOnce(Return(true))         // checkForUnexpectedStatus()
            .WillOnce(Return(true))         // closeDevicesIfNeeded()
            .WillRepeatedly(Return(false)); // canSetPowerState()
        ...
        // Monitor to set initial power state to on
        chassis->monitor(services);
        EXPECT_EQ(chassis->getPowerState(), PowerState::on);

        // Now simulate chassis disappearing: power off should be allowed
        auto [canSet,
    ...
    // Test where false: Chassis is not enabled and request is on: Should log
    // error.
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
    ...
    // Test where false: Chassis does not have input power and request is on:
    // Should log error.
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
        ...
        std::map<std::string, std::string> additionalData{
            {"CHASSIS_NUMBER", "1"},
            {"UNEXPECTED_CHASSIS_STATUS", "Chassis does not have input power"},
            {"POWER_STATE", "Unknown"},
            {"NEW_POWER_STATE", "on"}};
        EXPECT_CALL(services, logError(error, Entry::Level::Informational,
                                       additionalData))
    ...
    // Test where true: Chassis does not have input power, but request is off
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
        ...
        // Now simulate loss of input power: power off should be allowed
        auto [canSet,
              reason] = chassis->canSetPowerState(PowerState::off, services);
        EXPECT_TRUE(canSet);
        EXPECT_TRUE(reason.empty());
    ...
    // Test where false: Unable to determine chassis status: Should log error.
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
        ...
        std::map<std::string, std::string> additionalData{
            {"CHASSIS_NUMBER", "1"},
            {"UNEXPECTED_CHASSIS_STATUS",
             "Unable to determine chassis status: Present property value could not be obtained."},
            {"POWER_STATE", "Unknown"},
            {"NEW_POWER_STATE", "on"}};
        EXPECT_CALL(services, logError(error, Entry::Level::Informational,
                                       additionalData))
...
TEST(ChassisTests, SetPowerState)
{
    ...
    catch (const std::runtime_error& e)
    {
        EXPECT_STREQ(
            e.what(),
            "Unable to set chassis 1 to state on: Unable to determine chassis status: Present property value could not be obtained.");
    }
    ...
    // Test where works: Power off: Chassis not present; skips hardware powerOff
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
        ...
        EXPECT_CALL(monitor, isPresent)
            ...
            .WillOnce(Return(true))         // checkForUnexpectedStatus()
            .WillOnce(Return(true))         // closeDevicesIfNeeded()
            .WillRepeatedly(Return(false)); // canSetPowerState/setPowerState()
        ...
        // Simulate chassis disappearing; setPowerState(off) should succeed
        // without calling powerOff() on the hardware
        chassis->setPowerState(PowerState::off, services);
        EXPECT_EQ(chassis->getPowerState(), PowerState::off);
        EXPECT_TRUE(chassis->isInPowerStateTransition());
    }

    // Test where works: Power off: No input power; skips hardware powerOff
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
        ...
        // Simulate loss of input power; setPowerState(off) should succeed
        // without calling powerOff() on the hardware
        chassis->setPowerState(PowerState::off, services);
        EXPECT_EQ(chassis->getPowerState(), PowerState::off);
        EXPECT_TRUE(chassis->isInPowerStateTransition());
...
TEST(ChassisTests, Monitor)
{
    ...
    // Test where works: Power good is off for first monitor: Sets initial power
    // state and power good values: Chassis is powered on: Chassis status
    // becomes unexpected: Unexpected status error is detected and logged:
    // Devices are closed.
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
        ...
        // Monitor: Chassis is now not present, unexpected status error logged
        chassis->monitor(services);
        EXPECT_EQ(chassis->getPowerState(), PowerState::on);
        EXPECT_EQ(chassis->getPowerGood(), PowerGood::off);
    }
...
{
    // Verify PSU error and pgood fault are cleared
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
    ...
    // Verify unexpected status is cleared
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
        ...
        EXPECT_CALL(
            ...
            logErrorMsg(
                "Chassis 1 requested power state is on: Chassis is not present"))
            .Times(2);
        EXPECT_CALL(services, logError).Times(2);
        ...
        // Monitor again. It should not write to journal or log error again
        chassis->monitor(services);

        // Clear error history, which clears the unexpected status history
        chassis->clearErrorHistory();

        // Monitor again. It should write to journal and log error again.
        chassis->monitor(services);
    }
}
...
TEST(ChassisTests, GetPowerGoodTimeout)
{
    ...
    // Verify value is set in D-Bus interface
    {
        ...
        chassis->setPowerGoodTimeout(std::chrono::seconds{3});
        chassis->monitor(services);
        EXPECT_TRUE(chassis->hasDBusInterface());
        EXPECT_EQ(chassis->getDBusInterface().getPgoodTimeoutProperty(), 3);

        chassis->setPowerGoodTimeout(std::chrono::seconds{5});
        EXPECT_EQ(chassis->getDBusInterface().getPgoodTimeoutProperty(), 5);
    }
}
...
TEST(ChassisTests, GetObjectPath)
{
    size_t number{3};
    ...
    EXPECT_EQ(chassis.getObjectPath(), "/org/openbmc/control/power3");
}

TEST(ChassisTests, HasDBusInterface)
{
    // The test variations are covered by SetPowerGoodValue
}

TEST(ChassisTests, GetDBusInterface)
{
    // The test variations are covered by SetPowerGoodValue
}

TEST(ChassisTests, SetPowerStateValue)
{
    // The test variations are covered by SetPowerGoodValue
}

TEST(ChassisTests, SetPowerGoodValue)
{
    std::unique_ptr<Chassis> chassis = createChassis(1);
    MockServices services;
    ...
    // Power on chassis. Power good and D-Bus interface not valid.
    chassis->setPowerState(PowerState::on, services);
    EXPECT_EQ(chassis->getPowerState(), PowerState::on);
    EXPECT_THROW(chassis->getPowerGood(), std::runtime_error);
    EXPECT_FALSE(chassis->hasDBusInterface());
    ...
    EXPECT_EQ(chassis->getPowerGood(), PowerGood::on);
    EXPECT_TRUE(chassis->hasDBusInterface());
    EXPECT_EQ(chassis->getDBusInterface().getChassisNumber(), 1);
    EXPECT_EQ(chassis->getDBusInterface().getStateProperty(), 1);
    EXPECT_EQ(chassis->getDBusInterface().getPgoodProperty(), 1);
    EXPECT_EQ(chassis->getDBusInterface().getPgoodTimeoutProperty(),
              chassis->getPowerGoodTimeout().count() / 1000);

    // Power off chassis
    chassis->setPowerState(PowerState::off, services);
    EXPECT_EQ(chassis->getPowerState(), PowerState::off);
    EXPECT_EQ(chassis->getPowerGood(), PowerGood::on);
    EXPECT_EQ(chassis->getDBusInterface().getStateProperty(), 0);
    EXPECT_EQ(chassis->getDBusInterface().getPgoodProperty(), 1);
    ...
    EXPECT_EQ(chassis->getPowerGood(), PowerGood::off);
    EXPECT_EQ(chassis->getDBusInterface().getStateProperty(), 0);
    EXPECT_EQ(chassis->getDBusInterface().getPgoodProperty(), 0);
}

TEST(ChassisTests, CreateDBusInterfaceIfPossible)
{
    // The test variations are covered by SetPowerGoodValue
}
...
TEST(ChassisTests, CloseDevicesIfNeeded)
{
    // Test where not present: Devices are closed
    {
        std::unique_ptr<Chassis> chassis = createChassis(2);
        MockServices services;
    ...
    // Test where not available: Devices are closed
    {
        std::unique_ptr<Chassis> chassis = createChassis(2);
        MockServices services;
    ...
    // Test where input power not good: Devices are closed
    {
        std::unique_ptr<Chassis> chassis = createChassis(2);
        MockServices services;
    ...
    // Test where chassis status is good: Devices are not closed
    {
        std::unique_ptr<Chassis> chassis = createChassis(2);
        MockServices services;
...
TEST(ChassisTests, UpdatePowerGood)
{
    ...
    // Test where not present: Sets power good to off
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
        ...
        // Call monitor(). Chassis not present. Sets power good to off.
        chassis->monitor(services);
        EXPECT_EQ(chassis->getPowerGood(), PowerGood::off);
    }

    // Test where input power not good: Sets power good to off
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
        ...
        // Call monitor(). Input power not good. Sets power good to off.
        chassis->monitor(services);
        EXPECT_EQ(chassis->getPowerGood(), PowerGood::off);
    }

    // Test where not available: Does not set power good
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
    ...
    // Test where works: Sets power good to false
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
    ...
    // Test where works: Sets power good to true
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
...
TEST(ChassisTests, CheckForPowerGoodError)
{
    ...
    // State transition: Power off timeout
    {
        ...
        EXPECT_CALL(
            services,
            logInfoMsg("Chassis 1 power state is off and power good is on"))
            .Times(1);
        EXPECT_CALL(services,
    ...
    // Power good fault detection skipped: Invalid chassis status
    {
        ...
        EXPECT_CALL(device, close).Times(3);
...
TEST(ChassisTests, CheckForUnexpectedStatus)
{
    ...
    // Test where power state is on but chassis is not present: In state
    // transition: Verify journal message and error log not created twice.
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
        ...
        // Monitor again. Errors should not be logged again
        chassis->monitor(services);
    }

    // Test where power state is on but chassis does not have input power: Not
    // in state transition
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
        ...
        EXPECT_CALL(monitor, isInputPowerGood)
            ...
            .WillOnce(Return(true))         // updatePowerGood() again
            .WillOnce(Return(true))         // checkForPowerGoodError()
            .WillRepeatedly(Return(false)); // checkForUnexpectedStatus()
        ...
        // Monitor. Journal message should be written but no error log should be
        // created.
        chassis->monitor(services);
        EXPECT_EQ(chassis->getPowerState(), PowerState::on);
        EXPECT_EQ(chassis->getPowerGood(), PowerGood::on);
    ...
    // Test where power state is on but chassis is not available: In state
    // transition
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
        ...
        EXPECT_CALL(monitor, isAvailable)
            ...
            .WillOnce(Return(true)) // checkForPowerGoodError()
            .WillOnce(Return(true)) // closeDevicesIfNeeded()
            .WillOnce(Return(true)) // canSetPowerState() in setPowerState()
            .WillRepeatedly(Return(false));
    ...
    // Test where power state is on and chassis status is good
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
...
TEST(ChassisTests, LogUnexpectedStatusError)
{
    // Test where power state is not known: No additional error info
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
    ...
    // Test where power state is known: Additional error info
    {
        std::unique_ptr<Chassis> chassis = createChassis(1);
        MockServices services;
        ...
        // Check whether state can be set to off. Should result in error logged.
        auto [canSet,
              reason] = chassis->canSetPowerState(PowerState::off, services);
        EXPECT_FALSE(canSet);
        EXPECT_EQ(
