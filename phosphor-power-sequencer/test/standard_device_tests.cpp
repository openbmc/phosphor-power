...
/**
 * @class MockStandardDevice
 *
 * Mock implementation of the StandardDevice abstract class.
 *
 * This mock implementation is required for two reasons:
 ...
 */
class MockStandardDevice : public StandardDevice
{
  public:
    // Specify which compiler-generated methods we want
    MockStandardDevice() = delete;
    MockStandardDevice(const MockStandardDevice&) = delete;
    MockStandardDevice(MockStandardDevice&&) = delete;
    MockStandardDevice& operator=(const MockStandardDevice&) = delete;
    MockStandardDevice& operator=(MockStandardDevice&&) = delete;
    virtual ~MockStandardDevice() = default;

    // Constructor just calls StandardDevice constructor
    explicit MockStandardDevice(const std::string& name,
...
TEST(StandardDeviceTests, Constructor)
{
    ...
    {
        std::vector<std::unique_ptr<Rail>> rails{};
        MockStandardDevice device{"xyz_pseq", std::move(rails)};
    ...
    // Non-empty vector of rails
    {
        ...
        MockStandardDevice device{"abc_pseq", std::move(rails)};
...
TEST(StandardDeviceTests, HasPgoodFault)
{
    // No rail has a pgood fault
    {
        ...
        EXPECT_FALSE(device.hasPgoodFault(services, powerSupplyError, error,
                                          additionalData));
    ...
    // Is a PSU rail: No PSU error specified
    {
        ...
        std::string error{};
        std::map<std::string, std::string> additionalData{};
        EXPECT_TRUE(device.hasPgoodFault(services, powerSupplyError, error,
                                         additionalData));
        EXPECT_EQ(error,
    ...
    // Exception is thrown during pgood fault detection
    {
        ...
        try
        {
            device.hasPgoodFault(services, powerSupplyError, error,
                                 additionalData);
