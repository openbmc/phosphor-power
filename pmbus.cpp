...
std::string PMBus::getDeviceName()
{
    ...
    {
        log<level::ERR>((std::string("Unable to read PMBus device name PATH=") +
                         path.string())
                            .c_str());
    }
...
std::string PMBus::readString(const std::string& name, Type type)
{
    ...
    catch (std::exception& e)
    {
        auto rc = errno;
        log<level::ERR>(
            std::string("PMBus::readString: Failed to read sysfs file "
                        "FILENAME=" +
                        path.string())
                .c_str());
    ...
    if (file)
    {
        ...
        if (bytes != length)
        {
            ...
            else if (ferror(file.get()))
            {
                ...
                log<level::ERR>(
                    std::string(
                        "JJM:  PMBus::readBinary() Failed to read sysfs file "
                        "errno=" +
                        ...
                        Type type)
{
    ...
    {
        auto rc = errno;
        log<level::ERR>(std::string("Failed to write binary data to sysfs file "
                                    "FILENAME=" +
                                    path.string())
                            .c_str());
